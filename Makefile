...
PROD_OBJ += $(OBJ_DIR)/tcProcessor.o

# all rule: should be "[library] [executable] [test] [coverage] [dox]" 
all : test #executable coverage # library dox
...
test :  $(TST_DIR)/$(TESTNAME) #$(PROD_OBJ:$(OBJ_DIR)/%.o=$(TST_DIR)/%_test.xml)
...
$(OBJ_DIR)/%.o: $(SRC_DIR)/%.cpp
	@mkdir -p $(OBJ_DIR) $(LOG_DIR)
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) $(GCOVFLAGS) -c $(@:$(OBJ_DIR)/%.o=$(SRC_DIR)/%.cpp) -o $@ | tee -a $(LOG_DIR)/build.log
...
#$(TST_DIR)/$(TESTNAME) : $(PROD_OBJ) $(GMOCK_DIR)/make/gmock_main.a
$(TST_DIR)/$(TESTNAME) : $(PROD_OBJ) $(TEST_OBJ) $(GMOCK_DIR)/make/gmock_main.a
	@mkdir -p $(TST_DIR) $(LOG_DIR)
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) $(LINKFLAGS) -fprofile-arcs $^ -lgcov -o $@ | tee -a $(LOG_DIR)/build.log
...
$(TST_DIR)/coverage.xml : $(PROD_OBJ:.o=.gcda) $(PROD_OBJ:.o=.gcno)
	@cd $(OBJ_DIR) $(LOG_DIR)
	gcovr -x &> $@
